...
import { AlertTriangle, CheckCircle2, Loader2, Pause, Play, Truck } from 'lucide-react';
import { setShipdayDispatchActive } from '../actions';

export function ShipdayToggle({
  initialActive,
  configured,
  configError,
}: {
  ...
  function handlePause() {
    ...
    startTransition(async () => {
      const res = await setShipdayDispatchActive(false);
  ...
  function handleActivateConfirmed() {
    ...
    setError(null);
    startTransition(async () => {
      const res = await setShipdayDispatchActive(true);
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between rounded-xl border border-border bg-background px-4 py-4">
        <div className="flex items-center gap-3">
          ...
          >
            {active ? <Truck className="h-4 w-4" strokeWidth={2.5} /> : <Pause className="h-4 w-4" strokeWidth={2.5} />}
          </span>
          <div>
